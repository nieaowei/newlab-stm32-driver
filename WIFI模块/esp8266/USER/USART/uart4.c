...
void uart4_init(){
	...
	uart4_it.NVIC_IRQChannelSubPriority=3;
	uart4_it.NVIC_IRQChannelPreemptionPriority=3;
...
void UART4_IRQHandler(void){
	...
	if(USART_GetITStatus(UART4,USART_IT_RXNE)==SET){//如果是接收完成中断
		current=USART_ReceiveData(UART4);//读取接受到的数据存取current
		FIFO_In_Byte(&uart4_rx_fifo,current);//放到缓存队列里面
		//printf("%c",current);
	}
}
/***************************************************************
