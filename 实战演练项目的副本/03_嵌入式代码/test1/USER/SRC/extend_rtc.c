...
//常数定义
#define PCF8563_ADDR			0x51 			//PCF8563的IIC1地址
...
{
    uint8_t i,data[1]={address};
    
		IIC_Write_Bytes(PCF8563_ADDR,data,1);
		
    //IIC_Write_Byte(PCF8563_ADDR|0x01);      //第0位置1表示读操作
	
	
    for(i=0;i<count;i++)
    {
        IIC_ReadBytes_Ack(PCF8563_ADDR,&buf[i],1);
    }
}
...
void PCF8563_WriteByte(uint8_t address, uint8_t mdata)
{
	...
	data[0]=address;
	data[1]=mdata;
	IIC_Write_Bytes(PCF8563_ADDR,data,2);
}
